...
Compress-Archive -Path "$PSScriptRoot/FeedbackFunctionsApp/bin/Release/net9.0/publish/*" -DestinationPath "$publishedAppsDirectory/functions.zip"

Set-Location "$PSScriptRoot/FeedbackWebApp"
& "dotnet" publish -c Release
Copy-Item -Path "$PSScriptRoot/FeedbackWebApp/bin/Release/net9.0/publish/wwwroot/*" -Destination "$publishedAppsDirectory/webapp" -Recurse
