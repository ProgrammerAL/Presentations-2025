    ...
    ApiResources ApiResources)
{
    ...
    private SiteStorageInfra GenerateSiteInfra()
    {
        ...
        {
            var filesPath = $"{GlobalConfig.StaticSiteConfig.StaticSitePath}";
            var appsettingsFilePath = $"{filesPath}/appsettings.json";
            WriteConfigFiles(appsettingsFilePath, apiEndpoint);

            //Create Pulumi entries for each file to upload
            var fullSearchPath = Path.GetFullPath(filesPath);
